...
public final class KeyHandler implements DeviceKeyHandler {
    ...
    public static final String PERMISSION_KEYPAD_RECEIVER =
            "org.omnirom.asusec.permission.KEYPAD_RECEIVER";

    public static final String ACTION_DOCK_KEYPAD_KEY_PRESSED =
                                "org.omnirom.asusec.actions.ACTION_DOCK_KEYPAD_KEY_PRESSED";
    public static final String EXTRA_ASUSEC_KEY = "key";
    public static final String EXTRA_ASUSEC_STATUS = "status";
    public static final String EXTRA_ASUSEC_VALUE = "value";

    // Private asusec keys values (for notification purpose)
    public static final int ASUSEC_UNKNOWN          = -1;
    public static final int ASUSEC_WIFI             =  1;
    public static final int ASUSEC_BT               =  2;
    public static final int ASUSEC_TOUCHPAD         =  3;
    public static final int ASUSEC_BRIGHTNESS       =  4;
    public static final int ASUSEC_VOLUME           =  5;
    public static final int ASUSEC_VOLUME_MUTE      =  6;
    public static final int ASUSEC_MEDIA            =  7;
    public static final int ASUSEC_SCREENSHOT       =  8;
    public static final int ASUSEC_EXPLORER         =  9;
    public static final int ASUSEC_SETTINGS         = 10;
    public static final int ASUSEC_CAPS_LOCK        = 11;

    public static final int ASUSEC_STATUS_OFF       =  0;
    public static final int ASUSEC_STATUS_ON        =  1;

    public static final int ASUSEC_MEDIA_PLAY_PAUSE =  0;
    public static final int ASUSEC_MEDIA_PREVIOUS   =  1;
    public static final int ASUSEC_MEDIA_NEXT       =  2;
    ...
    public boolean handleKeyEvent(KeyEvent event) {
        ...
        // Asusec should only handle key events when the device is docked.
        ...
        switch (event.getScanCode()) {
            ...
            case SCANCODE_VOLUME_MUTE:
                ...
                // treat this event as a volume mute toggle action. the asusec KeyHandler
    ...
    private void toggleWifi() {
        ...
        if (state == WifiManager.WIFI_STATE_ENABLED
            ...
            notifyKey(ASUSEC_WIFI, ASUSEC_STATUS_OFF);

        } else if (state == WifiManager.WIFI_STATE_DISABLED
                && apState == WifiManager.WIFI_AP_STATE_DISABLED) {
            mWifiManager.setWifiEnabled(true);
            notifyKey(ASUSEC_WIFI, ASUSEC_STATUS_ON);
        }
    }
    ...
    private void toggleBluetooth() {
        ...
        if (state == BluetoothAdapter.STATE_OFF) {
            ...
            notifyKey(ASUSEC_BT, ASUSEC_STATUS_ON);
        }
        if (state == BluetoothAdapter.STATE_ON) {
            mBluetoothAdapter.disable();
            notifyKey(ASUSEC_BT, ASUSEC_STATUS_OFF);
        }
    }
    ...
    private void toggleTouchpad() {
        ...
        notifyKey(ASUSEC_TOUCHPAD, mTouchpadEnabled ? ASUSEC_STATUS_ON : ASUSEC_STATUS_OFF);
    }
    ...
    private void brightnessDown() {
        ...
        setBrightness(value);
        notifyKey(ASUSEC_BRIGHTNESS, ASUSEC_STATUS_OFF, value);
    }
    ...
    private void toggleAutoBrightness() {
        ...
        int status = currentValue == Settings.System.SCREEN_BRIGHTNESS_MODE_MANUAL ?
                                    ASUSEC_STATUS_ON :
                                    ASUSEC_STATUS_OFF;
        notifyKey(ASUSEC_BRIGHTNESS, status, value);
    }
    ...
    private void launchExplorer() {
        notifyKey(ASUSEC_EXPLORER, ASUSEC_STATUS_OFF);
    }

    private void launchSettings() {
        try {
            mContext.startActivity(mSettingsIntent);
            notifyKey(ASUSEC_SETTINGS, ASUSEC_STATUS_OFF);
    ...
    private void toggleAudioMute() {
        ...
        if (mAudioManager.getMode() == AudioManager.MODE_NORMAL) {
            ...
            int status = newValue == AudioManager.RINGER_MODE_NORMAL ?
                    ASUSEC_STATUS_OFF :
                    ASUSEC_STATUS_ON;
            notifyKey(ASUSEC_VOLUME_MUTE, status);
        }
    }
    ...
    private void volumeDown() {
        ...
        int status = mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_NORMAL ?
                    ASUSEC_STATUS_ON :
                    ASUSEC_STATUS_OFF;
        notifyKey(ASUSEC_VOLUME, status, value);
    }
    ...
    private void mediaPlayPause() {
        notifyKey(ASUSEC_MEDIA, ASUSEC_MEDIA_PLAY_PAUSE);
    }

    private void mediaPrevious() {
        notifyKey(ASUSEC_MEDIA, ASUSEC_MEDIA_PREVIOUS);
    }

    private void mediaNext() {
        notifyKey(ASUSEC_MEDIA, ASUSEC_MEDIA_NEXT);
    }
    ...
    private void capsLock(KeyEvent event) {
        int status = event.getMetaState() == KeyEvent.META_CAPS_LOCK_ON ?
                ASUSEC_STATUS_ON :
                ASUSEC_STATUS_OFF;
        notifyKey(ASUSEC_CAPS_LOCK, status);
    }
    ...
    private void takeScreenshot() {
        synchronized (mScreenshotLock) {
            ...
            notifyKey(ASUSEC_SCREENSHOT, ASUSEC_STATUS_OFF);
    ...
    private void notifyKey(int asuseckey, int status, int value) {
        Intent intent = new Intent();
        intent.setAction(ACTION_DOCK_KEYPAD_KEY_PRESSED);
        intent.putExtra(EXTRA_ASUSEC_KEY, asuseckey);
        intent.putExtra(EXTRA_ASUSEC_STATUS, status);
        intent.putExtra(EXTRA_ASUSEC_VALUE, value);
        mContext.sendBroadcast(intent, PERMISSION_KEYPAD_RECEIVER);
    }

    private void notifyKey(int asuseckey, int status) {
        notifyKey(asuseckey, status, 0);
    }
