...
import org.omnirom.asusec.DockEmbeddedController;
import org.omnirom.asusec.KeyHandler;
...
public class DeviceBroadcastReceiver extends BroadcastReceiver {
    ...
    private void checkKeyPadKeyPressed(Context ctx, Intent intent) {
        ...
        int key = intent.getIntExtra(
                KeyHandler.EXTRA_ASUSEC_KEY, KeyHandler.ASUSEC_UNKNOWN);
        int status = intent.getIntExtra(
                KeyHandler.EXTRA_ASUSEC_STATUS, KeyHandler.ASUSEC_STATUS_OFF);
        String title = null;
        int icon = com.android.internal.R.drawable.ic_settings_language;

        // Choose the notification from key
        switch (key) {
            case KeyHandler.ASUSEC_WIFI:
                title = ctx.getString(
                        (status == KeyHandler.ASUSEC_STATUS_ON) ?
                            R.string.kp_wifi_on :
                            R.string.kp_wifi_off);
                break;

            case KeyHandler.ASUSEC_BT:
                title = ctx.getString(
                        (status == KeyHandler.ASUSEC_STATUS_ON) ?
                            R.string.kp_bt_on :
                            R.string.kp_bt_off);
            ...
            case KeyHandler.ASUSEC_TOUCHPAD:
                title = ctx.getString(
                        (status == KeyHandler.ASUSEC_STATUS_ON) ?
                            R.string.kp_tp_on :
                            R.string.kp_tp_off);
            ...
            case KeyHandler.ASUSEC_BRIGHTNESS:
                if (status == KeyHandler.ASUSEC_STATUS_ON) {
                    title = ctx.getString(R.string.kp_brightness_auto);
                } else {
                    int value = intent.getIntExtra(KeyHandler.EXTRA_ASUSEC_VALUE, 0);
                    title = ctx.getString(R.string.kp_brightness_manual, value);
                }
                break;

            case KeyHandler.ASUSEC_VOLUME:
                int value = intent.getIntExtra(KeyHandler.EXTRA_ASUSEC_VALUE, 0);
                title = ctx.getString(R.string.kp_volume, String.valueOf(value));
                break;

            case KeyHandler.ASUSEC_VOLUME_MUTE:
                title = ctx.getString(
                        (status == KeyHandler.ASUSEC_STATUS_ON) ?
            ...
            case KeyHandler.ASUSEC_MEDIA:
                int res = R.string.kp_media_play_pause;
                if (status == KeyHandler.ASUSEC_MEDIA_PREVIOUS) {
                    res = R.string.kp_media_previous;
                } else if (status == KeyHandler.ASUSEC_MEDIA_NEXT) {
                    res = R.string.kp_media_next;
                }
                title = ctx.getString(res);
                break;

            case KeyHandler.ASUSEC_SCREENSHOT:
                title = ctx.getString(R.string.kp_screenshot);
                break;

            case KeyHandler.ASUSEC_EXPLORER:
                title = ctx.getString(R.string.kp_explorer);
                break;

            case KeyHandler.ASUSEC_SETTINGS:
                title = ctx.getString(R.string.kp_settings);
                break;

            case KeyHandler.ASUSEC_CAPS_LOCK:
                title = ctx.getString(
                        (status == KeyHandler.ASUSEC_STATUS_ON) ?
